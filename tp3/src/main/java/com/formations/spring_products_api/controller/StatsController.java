...
public class StatsController {
    ...
    @GetMapping("/products/average-price-by-category")
    public ResponseEntity<List<Map<String, Object>>> getAveragePriceByCategory() {
    ...
    @GetMapping("/orders/product-stats")
    public ResponseEntity<List<ProductOrderStats>> getMostOrderedProductStats(
        @RequestParam(defaultValue = "10") int limit
    ) {
        return ResponseEntity.ok(statsService.getMostOrderedProductStats(limit));
    }
    ...
    @GetMapping("/categories/with-min-products")
    public ResponseEntity<List<Category>> getCategoriesWithMinProducts(
        @RequestParam(defaultValue = "1") int minProducts
    ) {
        return ResponseEntity.ok(statsService.getCategoriesWithMinProducts(minProducts));
    }
}
